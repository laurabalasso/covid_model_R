...
data {
  ...
  matrix<lower=0, upper=1>[N, J] yellow;
  matrix<lower=0, upper=1>[N, J] orange;
  matrix<lower=0, upper=1>[N, J] red;
...
model {
  beta_yellow ~ normal(-1, 10);
  beta_orange~ normal(-1, 10);
  beta_red~ normal(-1, 10);
 // beta_school ~ normal(1, 10);
  ...
  log_rt[1, ] ~ normal(0, 5);
  for(n in 2:N){
    log_rt[n, ] ~ normal( alpha + log_rt[n-1, ] + beta_yellow * yellow[n, ] + beta_orange * orange[n, ]
    + beta_red * red[n, ], 0.035);
  }
