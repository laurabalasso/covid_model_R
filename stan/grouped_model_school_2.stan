...
data {
  ...
  vector<lower=0, upper=1>[N_nonzero] mask;
...
model {
  ...
  for(j in 1:J){
    nonzero_positives[ ,j ] ~ neg_binomial_2(eta[,j] + beta_school * school + beta_mask * mask, phi);
  }
...
generated quantities {
  ...
  for(n in 1:N_nonzero){
    for(j in 1:J){
      real eta_nj = eta[n, j] + beta_school * school[n] + beta_mask * mask[n];
