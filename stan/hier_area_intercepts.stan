...
data {
  ...
  int<lower=1, upper=4> risk_idx[N_nonzero, J];
}
...
model {
  ...
  for(j in 1:J){
    nonzero_positives[ ,j ] ~ neg_binomial_2(eta[,j]+ risk_level[risk_idx[,j]], phi);
  }
...
generated quantities {
  ...
  for(n in 1:N_nonzero){
    for(j in 1:J){
      real eta_nj = eta[n, j]+ risk_level[risk_idx[n,j]];
